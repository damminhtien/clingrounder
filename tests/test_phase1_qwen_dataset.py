...
def test_hard_negative_builder_emits_atomic_verifier_actions(tmp_path: Path) -> None:
    spans = tmp_path / "spans.jsonl"
    rows = [
        _span_row("1", "train", "Bệnh nhân đau ngực trái", 10, 23, "SYMPTOM"),
        _span_row("2", "development", "Có sốt", 3, 6, "SYMPTOM"),
    ]
    spans_sha256 = write_jsonl(spans, rows)
    manifest = tmp_path / "source-manifest.json"
    ...
    record = json.loads(
        (tmp_path / "output/hard_negatives.jsonl")
        .read_text(encoding="utf-8")
        .splitlines()[0]
    )
    assistant = json.loads(record["messages"][-1]["content"])
    assert assistant["decisions"] == [
        {
            "action": "REPLACE_BOUNDARY",
            "proposal_id": "xlmr-0",
            "replacement_text": "đau ngực trái",
            "replacement_type": None,
        }
    ]
    assert "confidence" not in assistant["decisions"][0]
    assert "evidence_quote" not in assistant["decisions"][0]
