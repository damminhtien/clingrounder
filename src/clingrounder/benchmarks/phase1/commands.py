...
from clingrounder.benchmarks.phase1.proposal_source_report import (
...
from clingrounder.benchmarks.phase1.joint_span_run import (
...
from clingrounder.mining.io import load_documents, write_json
from clingrounder.pipeline.parallel_batch import ParallelBackend
from clingrounder.schema.document import ClinicalDocument
from clingrounder.utils.hashing import sha256_file
from clingrounder.utils.io import read_jsonl, read_source_text
...
def calibrate_phase1_joint_span_command(args: argparse.Namespace) -> int:
    """Fit and pin genre/type Platt mappings from document-grouped OOF verifier scores."""

    observations = tuple(
        Phase1JointSpanCalibrationObservation.from_dict(row)
        for row in read_jsonl(args.observations)
    )
    ...
    write_json(output, calibration.to_dict())
    report["artifact"] = {
        "path": str(output),
