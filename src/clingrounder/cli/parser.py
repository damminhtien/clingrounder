...
from clingrounder.validation.profiles import ValidationProfile
...
def _kg_parser(commands: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    build.add_argument("--cache-dir", default=".cache/clingrounder/knowledge-graph")
...
def _terminology_parser(commands: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    build.add_argument("--cache-dir", default=".cache/clingrounder/terminology")
...
def _data_parser(commands: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    snapshot_freeze.add_argument("--hash-salt", default="clingrounder-snapshot-v1")
    snapshot_freeze.add_argument(
        "--dedup-mode",
        choices=("near", "exact"),
        default="near",
