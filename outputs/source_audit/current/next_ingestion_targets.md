...
- Full RxNorm monthly release `RxNorm_full_06012026.zip` is not local. Official KSS URL redirects to UMLS login, so this needs a licensed UTS-authenticated download.
...
1. Add a licensed local full RxNorm zip and rerun `scripts/import_rxnorm_dictionary.py` with both `--prescribable-rxnorm` and `--full-rxnorm`.
